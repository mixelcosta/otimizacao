...
| **Tick de sensores no thread pool** | `DispatcherTimer.Tick` dispara a cada 500ms na UI thread. Para não bloquear a renderização, `TickSensoresAsync` (ShellViewModel) envolve a leitura IPC em `Task.Run` e usa `Interlocked.CompareExchange` para descartar ticks sobrepostos se o anterior ainda não concluiu. |
| **Callbacks diretos em vez de ObservableCollection no Dashboard** | As coleções `HistCpuTemp/GpuTemp/Clock` foram substituídas por `Action<double>?` no `DashboardViewModel`. A `ObservableCollection` causava `RemoveAt(0)` O(n) + 2 `CollectionChanged` por canal a cada tick (8 eventos/tick = GC e marshal desnecessários). O `RealtimeChartControl` mantém seu próprio buffer circular de 60 amostras. |
| **Brushes/pens em cache no RealtimeChartControl** | `FundoBrush`, `TextBrush` e `GradePen` são `static readonly` (criados uma vez por processo). `_linhaPen` por instância é recriado apenas em `OnPropertyChanged` quando `LinhaCor` muda — antes eram alocados a cada `Render()`. |
| **Leitores de sensores em paralelo** | `LeitorSensoresComposto` usa `Task.WhenAll` em vez de `foreach` sequencial. LHM, ADL e WMI possuem `SemaphoreSlim` internos e são thread-safe; executar em paralelo reduz a latência de `T_lhm + T_adl + T_wmi` para `max(T_lhm, T_adl, T_wmi)`. |
