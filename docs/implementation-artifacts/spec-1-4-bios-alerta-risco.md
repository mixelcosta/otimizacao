...
**Nota de correção pós-implementação (revisão independente `bmad-code-review`):** a revisão independente (4 lenses: Blind Hunter, Edge Case Hunter, Verification Gap, Acceptance Auditor) sobre o commit do Dev encontrou dois defeitos reais confirmados por dois revisores diferentes, corrigidos: (5) `VerGuiaBios()` revelava o guia sem checar `ConfirmadoBios` no próprio ViewModel — dependia inteiramente do botão do `ConfirmationPanel` estar desabilitado na View, sem defesa em profundidade (o driver já tem esse guard explícito em `AplicarAtualizacaoAsync`); corrigido com `if (!ConfirmadoBios) return;`. (6) `VerificarBiosAsync` atualizava `InfoBiosAtual` a cada nova verificação mas nunca resetava `PainelConfirmacaoBiosAberto`/`ConfirmadoBios`/`GuiaBiosVisivel` — uma confirmação/guia já revelados numa verificação anterior sobreviviam a um novo alerta, violando o espírito do "aparece toda vez" do Boundaries §Always; corrigido resetando os três no fim de cada `VerificarBiosAsync`. Também foi adicionado um botão manual "Verificar BIOS" (antes só havia verificação automática via `PopularBios`) para consistência com a seção de Software e como via de recuperação pros cenários de reentrância/dado obsoleto já deferidos, e corrigidos dois comentários desatualizados (`ConfirmationPanel.axaml.cs` ainda dizia "só Driver é usada nesta história"; `VerificadorBios.cs`/spec citavam `VersaoBios.Comparar` quando o código usa `VersaoBios.EhMaisRecente`).
