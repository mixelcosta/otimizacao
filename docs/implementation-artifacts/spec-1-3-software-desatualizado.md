...
A revisão independente `bmad-code-review` (4 lenses: Blind Hunter, Edge Case Hunter, Verification Gap, Acceptance Auditor) rodou sobre o commit resultante e encontrou mais dois defeitos reais, corrigidos: (3) o `catch (Exception ex)` em `VerificadorSoftware.VerificarAsync` também engolia `OperationCanceledException`/cancelamento do `CancellationToken`, tratando-o como "falha do provedor" em vez de propagar — corrigido com um `catch (OperationCanceledException) { throw; }` antes do catch genérico; (4) os guards de versão nula/vazia usavam `IsNullOrEmpty`, deixando passar uma versão só-com-espaços como "válida" — corrigido para `IsNullOrWhiteSpace` nos dois lados da comparação (`programa.Versao` e `oficial.VersaoDisponivel`). Também endureceu defensivamente `AbrirDownloadSoftware` (checa `IsNullOrEmpty` em vez de só `is null`). O achado de maior impacto potencial — a linha de wiring `Drivers.PopularProgramas(...)` em `ShellViewModel.cs` não ter nenhum teste — foi avaliado e deferido: é uma lacuna de infraestrutura pré-existente em todo o arquivo `ShellViewModel` (sem harness de teste headless do Avalonia no projeto), não uma regressão desta história; ver `deferred-work.md`.
